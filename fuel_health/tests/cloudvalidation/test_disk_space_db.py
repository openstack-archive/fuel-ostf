...
class DBSpaceTest(cloudvalidation.CloudValidationTest):
    ...
    def test_db_expectation_free_space(self):
        ...
        for host in self.controllers:
            if self._check_db_disk_expectation_warning(host):
                hosts.append(host)

        self.verify_response_true(not len(hosts),
                                  ''.join("Free disk space cannot be used ",
                                          "by database on node(s)").format(
                                  unicode(hosts)), 1)
