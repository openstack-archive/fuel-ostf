...
class VMBootTest(cloudvalidation.CloudValidationTest):
    ...
    def _check_host_boot(self, host):
        ...
        err_msg = ('The option "resume_guest_state_on_host_boot" '
                   'is set to True at compute node {host}, so it can be broken'
                   'down by the paused VMs after host boot.').format(host=host)
        ...
        self.verify_response_true(not out and not err
                                  or ('True' not in out
                                  and 'True' not in err),
                                  err_msg, 1)
