...
class TestMysqlReplication(BaseMysqlTest):
    ...
    def tearDownClass(cls):
        if cls.master_ip:
            try:
                cmd = ("sudo -i mysql -h localhost "
                       "-e 'DROP DATABASE %s'") % cls.database
    ...
    def test_mysql_replication(self):
        ...
        create_record = (
            'sudo -i mysql -h localhost -e "INSERT INTO '
            '{database}.{table} (data) '
        ...
        drop_db = (
            "sudo -i mysql -h localhost "
            "-e 'DROP DATABASE {database}'".format(database=database_name)
        )
