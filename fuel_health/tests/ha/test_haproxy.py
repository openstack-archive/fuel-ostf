...
class HAProxyCheck(test.BaseTestCase):
    ...
    def _check_haproxy_backend(remote,
        ...
        grep = ['|egrep "{}"'.format('|'.join(n)) for n in pos_filter if n]
        grep.extend(
            ['|egrep -v "{}"'.format('|'.join(n)) for n in neg_filter if n])

        return remote.exec_command("{}{}".format(cmd, ''.join(grep)))

    def test_001_check_state_of_backends(self):
        """Check HAProxy backends state,
        Test verifies state of HAProxy backends of environment
        Target Service: HAProxy

        Scenario:
            1. Ssh on each controller and get state of HAProxy backends
            2. For each backend check state
        Duration: 60 s.
        Available since release: 2014.2.2-7.0
        """
        ...
        for controller in self.controllers:
            ...
            if 'neutron' in self.config.network.network_provider:
                haproxy_status = self.verify(
                    10, self._check_haproxy_backend,
                    1,
            ...
            dead_backends = haproxy_status.splitlines()
            self.verify_response_true(
                len(dead_backends) == 0,
                "Step 2 failed: Controller has dead"
                "backend {}".format(dead_backends))
