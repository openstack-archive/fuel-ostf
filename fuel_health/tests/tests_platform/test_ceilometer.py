...
class CeilometerApiPlatformTests(ceilometermanager.CeilometerBaseTest):
    ...
    def test_check_volume_notifications(self):
        ...
        msg = 'creating volume'
        volume = self.verify(60, self._create_volume, 3,
                             fail_msg, msg, self.volume_client)

        fail_msg = 'Failed while waiting for "ACTIVE" status of volume.'
        msg = 'waiting for "Available" status of volume'
        self.verify(200, self.wait_for_volume_status, 4,
                    fail_msg, msg, volume, 'available')

        fail_msg = 'Failed to create volume snapshot.'
        msg = 'creating volume snapshot'
        snapshot = self.verify(60, self._create_snapshot, 5,
                               fail_msg, msg, self.volume_client, volume.id)

        fail_msg = 'Failed while waiting for "ACTIVE" status of snapshot.'
        msg = 'waiting for "Available" status of snapshot'
        self.verify(200, self.wait_for_snapshot_status, 6,
                    fail_msg, msg, volume, 'available')

        fail_msg='Failed to manipulate volume'
        msg = 'manipulating successfull'
        self.verify(300, self.volume_helper, 7,
                    fail_msg, msg, volume, instance)
        ...
        self.verify(300, self.wait_metrics, 8,
                    fail_msg, msg,
        ...
        self.verify(300, self.wait_metrics, 9,
