...
class SanityInfrastructureTest(nmanager.SanityChecksTest):
    ...
    def test_001_services_state(self):
        ...
        output = self.verify(50, self._list_services, 1,
                             "'nova service-list' command execution failed. ",
                             "'nova service-list' command execution",
                             (self.controller_names[0], )
                             )

        states = [svc.state for svc in output]
        LOG.debug(states)
        try:
            self.verify_response_true(
                downstate not in states,
            ...
            output = self._list_services(self.controller_names[0])
            states =[svc.state for svc in output]
            LOG.debug(states)
            self.verify_response_true(
                downstate not in states,
