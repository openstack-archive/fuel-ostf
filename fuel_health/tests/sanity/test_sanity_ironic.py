...
class IronicSmokeTests(ironicmanager.IronicTest):
    ...
    def test_ironic_node_actions(self):
        ...
        # self.node = self.node_update(self.ironic_client, self.node, prop, value_prop)
        fail_msg = ("Can't update node with properties. Please refer to "
                    "Openstack logs for more information.")
        self.node = self.verify(100, self.node_update, 2, fail_msg,
                                 'Updating node', self.ironic_client,
                                 self.node, prop, value_prop)
        LOG.debug(self.node)
        # ToDo Step 3
        # Step 4
        #self.node = self.node_delete(self.ironic_client, self.node)
