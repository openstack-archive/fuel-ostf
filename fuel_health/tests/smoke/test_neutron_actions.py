...
class TestNeutron(neutronmanager.NeutronBaseTest):
    ...
    def test_check_neutron_objects_creation(self):
        ...
        self.verify(600, self._check_vm_connectivity, 11,
                    "VM connectivity via IPv4 doesn`t function properly.",
                    'VM connectivity via IPv4 checking', ip_address,
                    30, (9, 60))

        self.verify(600, self._check_connectivity_from_vm, 12,
                    "Connectivity to gateway_ip from the VM via IPv6 "
                    "doesn`t function properly.",
                    "Connectivity checking to gateway_ip from the VM via IPv6",
                    ip_address,
                    30, (9, 60),
        ...
        self.verify(40, self._remove_router, 17, "Router can not be deleted",
                    "router deletion",
                    router, [subnet_v4['id'], subnet_v6['id']])
        self.verify(20, self._remove_subnet, 18,
                    "Subnet v4 can not be deleted",
                    "Subnet v4 deletion", subnet_v4)
        self.verify(20, self._remove_subnet, 19,
                    "Subnet v6 can not be deleted",
                    "Subnet v6 deletion", subnet_v6)
