...
class VanillaTwoClusterTest(SaharaClusterTest):
    ...
    def test_vanilla_two_cluster(self):
        ...
        Scenario:
            ...
            5. Check SSH connection to cluster nodes
        ...
        self.verify(self.process_timeout, self.check_hadoop_services, 4,
                    fail_msg, msg, cluster_id, self._processes_map)

        fail_msg = 'Failed to establish SSH connection to cluster nodes.'
        msg = 'establishing SSH connection to cluster nodes'
        self.verify(
            30, self.check_ssh_connection, 5, fail_msg, msg, cluster_id)
