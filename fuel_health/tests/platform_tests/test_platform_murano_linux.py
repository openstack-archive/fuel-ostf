...
class MuranoDeploymentSmokeTests(murano.MuranoTest):
    ...
    Special requirements:
        ...
        4. Demo image with Murano metadata should be imported.
        5. Linux image with Murano metadata should be imported.
    """
    ...
    def test_deploy_demo_service(self):
        ...
        msg = ("Demo image with Murano tag isn't available. "
               "This image should be imported into glance during the "
               "Open Stack deployment by default. "
        ...
        demo_image = self.find_murano_image('cirros.demo')
        if not demo_image:
            LOG.debug(msg)
            self.fail(msg)
