...
class CeilometerBaseTest(fuel_health.nmanager.NovaNetworkScenarioTest):
    ...
    def identity_helper(self):
        ...
        self.identity_v3_client.roles.update(
            role, user=self.identity_v3_client.user_id,
            project=self.identity_v3_client.project_id)
        self.identity_v3_client.roles.grand(
            role, user=self.identity_v3_client.user_id,
            project=self.identity_v3_client.project_id)
        ...
        trust = self.identity_v3_client.trusts.create(
            user,
            self.identity_v3_client.users.get(self.identity_v3_client.user_id),
            [role.name], tenant)
