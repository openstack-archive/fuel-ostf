...
class SavannaTest(nmanager.OfficialClientTest):
    ...
    def _ssh_command(self, cmd):
        ...
        try:
            ...
            output_err = stderr.read()
            ssh.close()
            LOG.debug('Remote ssh command: %s', cmd)
    ...
    def _check_auto_assign_floating_ip(self):
        ...
        cmd_quantum = ('grep network_api_class=nova.network.quantumv2.api.API '
                       '/etc/nova/nova.conf')
        output_nova, output_nova_err = self._ssh_command(cmd_nova)
        output_quantum, output_quantum_err = self._ssh_command(cmd_quantum)
