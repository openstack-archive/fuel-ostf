...
class SavannaTest(nmanager.OfficialClientTest):
    ...
    def _run_ssh_cmd(self, cmd):
        ...
        try:
            sshclient = ssh.Client(self.host[0], self.usr, self.pwd,
                                   key_filename=self.key, timeout=self.timeout)
            return sshclient.exec_longrun_command(cmd, return_stderr=True)
    ...
    def _try_port(self, host, port):
        ...
        while True:
            ...
            output, output_err = _run_ssh_cmd(cmd)
    ...
    def _check_auto_assign_floating_ip(self):
        ...
        cmd_neutron = ('grep -E '
                       ...
                       '/etc/nova/nova.conf')
        output_nova, output_nova_err = _run_ssh_cmd(cmd_nova)
        output_neutron, output_neutron_err = _run_ssh_cmd(cmd_neutron)
    ...
    def _await_active_workers_for_namenoded(self, node_info, plugin_name):
        ...
        _run_ssh_cmd('echo "%s" > /tmp/ostf-savanna.pem' %
                    self.keys[0].private_key)
        _run_ssh_cmd('chmod 600  /tmp/ostf-savanna.pem')
        while True:
            cmd = ('ssh -i /tmp/ostf-savanna.pem -l %s '
                   ...
                   (self.V_NODE_USERNAME, node_info['namenode_ip'],
                    hadoop_user))
            stdout, stderr = _run_ssh_cmd(cmd)
