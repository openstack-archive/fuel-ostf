...
class PlatformServicesBaseClass(NovaNetworkScenarioTest):
    ...
    def delete_resource(self, delete_method, get_method=None, timeout=300,
        ...
        try:
            ...
            err_msg = "Method for delete:{} has error state. Trace:{}"
            LOG.warn(err_msg.format(delete_method, exc))
        if get_method:
            self._wait_for_deletion(get_method, timeout, sleep)
        msg = 'Run method {} for delete has successful state.'
        LOG.debug(msg.format(get_method))
