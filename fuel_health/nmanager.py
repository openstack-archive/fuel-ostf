...
from keystoneauth1.identity import v3 as keystoneauth1_v3
...
class OfficialClientManager(fuel_health.manager.Manager):
    ...
    def _get_aodh_client(self, version='2'):
        ...
        keystone = self._get_identity_client(username, password, tenant)
        auth_url = self.config.identity.uri
        helper_list = auth_url.rstrip("/").split("/")
        helper_list[-1] = "v3/"
        auth_url = "/".join(helper_list)
        auth = keystoneauth1_v3.Password(
            ...
            project_domain_id=keystone.project_domain_id)
        sess = keystoneauth1_session.Session(auth=auth)
