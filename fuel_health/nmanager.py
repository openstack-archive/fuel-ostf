...
class PlatformServicesBaseClass(NovaNetworkScenarioTest):
    ...
    # Method for checking whether or not Sahara resource is deleted.
    def is_sahara_resource_deleted(self, resource_client, resource_id):
        """This method checks whether or not a Sahara resource is deleted.

        The API request is wrapped in the try/except block to correctly handle
        the "404 Not Found" exception. If the Sahara resource doesn't exist,
        this method will return True. Otherwise it will return False.
        """

        try:
            resource_client.get(resource_id)
        except saharaclient.api.base.APIException as sahara_api_exc:
