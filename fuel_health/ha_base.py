        ...
        if 'with-rsc' in constraints[rsc]:
            ...
            if 'score' in constraints[rsc]:
                if constraints[rsc]['score'] == '-INFINITY':
                    ...
                    # parent resource is started, then 'parent_started'
                    # nodes should be added to 'disallowed' nodes.
#                    disallowed = (set(disallowed) | set(parent_started))
                    ...
                    allowed = list(set(parent_started) &
                                   set(parent_allowed) &
                                   set(allowed))
                    # Extend 'disallowed' list with nodes where the parrent
                    # resource is running but not allowed
