...
def _get_cluster_attrs(cluster_id, token=None):
    ...
    comp_names = ['sahara', 'heat', 'ceilometer', 'ironic']
    ...
    murano = additional_components.get('murano', None)
    murano_artifacts = (response['editable']['murano_settings']
                        ['murano_glance_artifacts_plugin'].get('value', None))
    detach_murano = response['editable'].get('detach-murano', None)
    murano_plugin_enabled = None
    if detach_murano:
        murano_plugin_enabled = detach_murano['metadata'].get('enabled')
        if murano_plugin_enabled:
            additional_depl_tags.add('murano_plugin')

    # TODO(freerunner): Rework GLARE discover mechanism after
    # TODO removal murano from the box
    if murano and murano['value'] and murano_artifacts:
