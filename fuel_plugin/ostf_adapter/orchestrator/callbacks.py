...
def start_testruns_callback(body, message):
    LOG.info('Entering start_testrun_callback for message with body {0}'
             .format(body))
    ...
    try:
        ...
        LOG.error('Exception while starting testrun. '
                  'Details: body of message -- {0}, exc -- {1}'
                  .format(body, e.message))
        to_be_acknowledged = True
    finally:
        if to_be_acknowledged:
            LOG.info('Acknowledging message '
                     'with body {0} in start_testrun_callback'
                     .format(body))
            message.ack()
        else:
            LOG.info('Requeueing message with body {0}'.format(body))
