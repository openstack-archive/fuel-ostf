...
def get_description(test_obj):
    ...
    if isinstance(test_obj, case.Test):
        ...
        if docstring:
            ...
            rel_vers_pattern = "Available for release:.?(?P<rel_vers>.+)"
            docstring, rel_vers = _process_docstring(
                docstring,
                rel_vers_pattern
            )
            if rel_vers:
                test_data["release_version"] = rel_vers
...
def tests_availability_cond(cluster_data, test_entity_data):
    is_rel_ver_suitable = (
        True if not test_entity_data['release_version'] else
        _compare_release_versions(cluster_data['release_version'],
                                  test_entity_data['release_version'])
    )

    cond = (
        is_rel_ver_suitable
        and
        _process_deployment_tags(cluster_data['deployment_tags'],
