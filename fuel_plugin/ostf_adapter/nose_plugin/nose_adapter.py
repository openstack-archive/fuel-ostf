    ...
    def _run_tests(self, lock_path, dbpath,
        ...
        with engine.contexted_session(dbpath) as session:
            ...
            if os.path.exists(lock_path):
                present_lock = [
                    f for f in os.listdir(lock_path)
                    if os.path.isfile(os.path.join(lock_path, f))
                    and not
                    set(f.split('_'))
                    .isdisjoint(set(testrun.testset.exclusive_testsets))
                ]
            else:
            ...
            lock_to_aquire = None
            if present_lock:
                lock_to_aquire = present_lock.pop()
            elif testrun.test_set.exclusive_testsets:
                lock_to_aquire = '_'.join(
                    testrun.test_set.exclusive_testsets
                )

            fd = None
            if lock_to_aquire:
                fd = open(lock_to_aquire, 'w')
