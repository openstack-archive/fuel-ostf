...
''' Since our orm models classes has word 'Test' in
their names they cannot be imported at the module's
level otherwise these entities will be discovered by
nosetest engine and counted as test suits to be
executed and that obviously leads to errors.

To avoid the situation imports of model classes
are done inside of methods of actual test class here.

Please, note that such behavior is observed only for
nosetest, using pytest as tests's executor doesn't produce
such problem
'''
...
class TestModelTestMethods(base.BaseIntegrationTest):
    ...
    def setUp(self):
        ...
        # FIXME(aroma): remove these imports when pytest will be
        # default testing framework for ostf
        from fuel_plugin.ostf_adapter.storage.models import Test
        from fuel_plugin.ostf_adapter.storage.models import TestRun

        self.test_obj = self.session.query(Test)\
    ...
    def test_to_check(self):
        # FIXME(aroma): remove this import when pytest will be
        # default testing framework for ostf
        from fuel_plugin.ostf_adapter.storage.models import Test
    ...
    def test_update_only_running_tests(self):
        ...
        # FIXME(aroma): remove this import when pytest will be
        # default testing framework for ostf
        from fuel_plugin.ostf_adapter.storage.models import Test as Model
        Model.add_result(self.session, self.test_run.id,
                         self.test_obj.name,
                         {'status': expected_status})

        Model.update_running_tests(self.session, self.test_run.id)
    ...
    def test_update_test_run_tests_default_status(self):
        ...
        Model.update_test_run_tests(self.session, self.test_run.id,
                                    [self.test_obj.name])
